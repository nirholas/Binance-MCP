...
export function registerBinanceWalletAllCoinsInformation(server: McpServer) {
  server.registerTool(
    ...
    {
      description: "Get information for all coins.",
      inputSchema: {
    ...
    async ({ recvWindow }) => {
      try {
        ...
        const response = await (walletClient as any).restAPI.allCoinsInformation(params);
        ...
        return {
          content: [{ type: "text", text: `Failed to retrieve coin information: ${errorMessage}` }],
          isError: true,
        };
      }
    },
