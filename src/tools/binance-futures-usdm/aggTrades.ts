...
export function registerBinanceFuturesUSDMAggTrades(server: McpServer) {
  server.registerTool(
    ...
    {
      description: "Get compressed, aggregate trades for a specific USD-M Futures trading pair.",
      inputSchema: {
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Failed to retrieve USD-M Futures aggregated trades: ${errorMessage}`,
            },
          ],
          isError: true,
        };
