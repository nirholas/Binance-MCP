...
export function registerBinanceFuturesCOINMAggTrades(server: McpServer) {
  server.registerTool(
    ...
    {
      description: "Get compressed, aggregate trades for a specific COIN-M Futures trading pair.",
      inputSchema: {
        ...
        startTime: z
          .number()
          .optional()
          .describe("Timestamp in ms to get aggregate trades from INCLUSIVE"),
        endTime: z
          .number()
          .optional()
          .describe("Timestamp in ms to get aggregate trades until INCLUSIVE"),
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Failed to retrieve COIN-M Futures aggregated trades: ${errorMessage}`,
            },
          ],
          isError: true,
        };
