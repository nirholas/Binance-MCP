 ...
 * @license Apache-2.0
 */
// src/tools/binance-crypto-loans/flexible-api/getRepayHistory.ts
...
export function registerBinanceCryptoLoansFlexibleRepayHistory(server: McpServer) {
  server.registerTool(
    ...
    async (params) => {
      try {
        const response = await cryptoLoanClient.restAPI.getFlexibleLoanRepaymentHistory({
        ...
        return {
          content: [
            {
              type: "text",
              text: `Flexible Loan Repay History:\n${JSON.stringify(data, null, 2)}`,
            },
          ],
        };
      } catch (error) {
        ...
        return {
          content: [{ type: "text", text: `❌ Failed to get repay history: ${errorMessage}` }],
          isError: true,
        };
      }
    },
