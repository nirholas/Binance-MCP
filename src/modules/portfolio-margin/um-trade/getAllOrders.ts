 ...
 * @license Apache-2.0
 */
// src/modules/portfolio-margin/um-trade/getAllOrders.ts
...
export function registerPortfolioMarginUmGetAllOrders(server: McpServer) {
  server.tool(
    ...
    async (params) => {
      try {
        const response = await portfolioMarginClient.restAPI.umAllOrders({
        ...
        return {
          content: [
            {
              type: "text",
              text: `❌ Failed to get Portfolio Margin UM orders: ${errorMessage}`,
            },
          ],
          isError: true,
        };
