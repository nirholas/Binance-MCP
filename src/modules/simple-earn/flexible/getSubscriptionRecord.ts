 ...
 * @license Apache-2.0
 */
// src/modules/simple-earn/flexible/getSubscriptionRecord.ts
...
export function registerSimpleEarnFlexibleSubscriptionRecord(server: McpServer) {
  server.registerTool(
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `📜 Flexible Subscription Records\n\n${JSON.stringify(data, null, 2)}`,
            },
          ],
        };
      } catch (error) {
        ...
        return {
          content: [
            {
              type: "text",
              text: `❌ Failed to get subscription records: ${errorMessage}`,
            },
          ],
          isError: true,
        };
